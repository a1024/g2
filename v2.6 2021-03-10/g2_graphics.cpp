		...
		if(SSE4_1)
		{
			...
			for(int x=0;x<xEnd;x+=4)
			{
				...
				//	__m128 wbk=_mm_set_ps(wbuffer+pos.m128i_i32[0], wbuffer+pos.m128i_i32[0]
					if(m128i_i32(c1)&&m128_f32(A)>wbuffer[m128i_i32(pos)])
						gBitmap.rgb[m128i_i32(pos)]=lineColor, wbuffer[m128i_i32(pos)]=m128_f32(A);
					if(m128i_i32(c1, 1)&&m128_f32(A, 1)>wbuffer[m128i_i32(pos, 1)])
...
//	}
//}
void			point_3D_cam(Camera const &cam, dvec3 const &cp, int lineColor)
{
	const int bx1=0, bx2=w, by1=0, by2=h, bw=w, bh=h;
	if(cp.z>0)
	{
...
//void			point_3D_2x2(Camera const &cam, dvec3 const &p_world, int Rcolor)
//{
//	dvec3 cp;
//	cam.world2cam(p_world, cp);
...
//	}
//}
//void			point_3D_2x2(Camera const &cam, dvec3 const &p_world, int Rcolor, int Icolor, int Jcolor, int Kcolor)
//{
//	dvec3 cp;
//	cam.world2cam(p_world, cp);
//	//double dx=x-camx, dy=y-camy, dz=z-camz, cpt=dx*cax+dy*sax, Xcp1=dx*sax-dy*cax, Ycp1=cpt*say-dz*cay;
		...
		for(int ys=0;ys<h;++ys)
		{
			...
			//	__m128 wb=_mm_loadu_ps(wb_y+xs);
				...
				if(m128i_i32(c_or))
				{
					...
					__m128i m_tx=_mm_set_epi32(txk[m128i_i32(tx_idx, 3)], txk[m128i_i32(tx_idx, 2)], txk[m128i_i32(tx_idx, 1)], txk[m128i_i32(tx_idx)]);
					__m128i m_rgb=_mm_loadu_si128((__m128i*)(rgb_y+xs));
					__m128i m_rgb2=_mm_and_si128(m_rgb, c_result2);
...
const float	_pi=acos(-1.f), _2pi=2*_pi, pi_2=_pi*0.5f, inv_2pi=1/_2pi, sqrt2=sqrt(2.f), torad=_pi/180,
#ifdef __GNUC__
	infinity=INFINITY,
#else
...
void			send_color(unsigned location, int color)
{
	...
	glUniform4f(location, m128_f32(c), m128_f32(c, 1), m128_f32(c, 2), m128_f32(c, 3));
...
void			send_color_rgb(unsigned location, int color)
{
	...
	c=_mm_cvtepi32_ps(_mm_castps_si128(c));
	c=_mm_mul_ps(c, m_255);
	glUniform3f(location, m128_f32(c), m128_f32(c, 1), m128_f32(c, 2));
}
...
void			point_3D(Camera const &cam, dvec3 const &p_world, int color)
{
	if(usingOpenGL)
	{
		vec3 p=p_world;
