...
const char*		loadresource(int name, int type, int &size)//https://stackoverflow.com/questions/2933295/embed-text-file-in-a-resource-in-a-native-windows-application
{
	HRSRC rc=FindResourceA(ghInstance, MAKEINTRESOURCEA(name), MAKEINTRESOURCEA(type));
	if(!rc)
	{
		SYS_CHECK();
		size=0;
	...
	HGLOBAL rcData=LoadResource(ghInstance, rc);
	if(!rcData)
	{
		SYS_CHECK();
		size=0;
	...
	size=SizeofResource(ghInstance, rc);
	if(!size)
	{
		SYS_CHECK();
		return nullptr;
	}
	return (const char*)LockResource(rcData);//doesn't set last error
}