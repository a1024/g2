...
struct Vect2d
{
	...
	void setzero(){v=zero;}
	double& lo(){return m128d_v0(v);}
	double& hi(){return m128d_vn(v, 1);}
	double lo()const{return m128d_v0(v);}
	double hi()const{return m128d_vn(v, 1);}
	double& get(int component){return m128d_vn(v, component);}
	double get(int component)const{return m128d_vn(v, component);}
...
//	Vect2d r_is_true()const{return _mm_xor_pd(_mm_cmpeq_pd(v, zero), minus_one);}
	...
	{
		__m128d v_q=_mm_div_pd(v, q.v);
		m128d_v0(v_q)=std::floor(m128d_v0(v_q)), m128d_vn(v_q, 1)=std::floor(m128d_vn(v_q, 1));
		v_q=_mm_mul_pd(v_q, q.v);
		v=_mm_sub_pd(v, v_q);
		return *this;
	}
	...
	{
		m128d_v0(v)=std::ceil(m128d_v0(v)), m128d_vn(v, 1)=std::ceil(m128d_vn(v, 1));
		return *this;
	}
	Vect2d ceil()const
	{
	...
	{
		Vect2d x;
		m128d_v0(x.v)=std::ceil(m128d_v0(v)), m128d_vn(x.v, 1)=std::ceil(m128d_vn(v, 1));
		return x;
	}
	Vect2d& floor_this()
	{
	...
	{
		m128d_v0(v)=std::floor(m128d_v0(v)), m128d_vn(v, 1)=std::floor(m128d_vn(v, 1));
		return *this;
	}
	Vect2d floor()const
	{
	...
	{
		Vect2d x;
		m128d_v0(x.v)=std::floor(m128d_v0(v)), m128d_vn(x.v, 1)=std::floor(m128d_vn(v, 1));
		return x;
	}
	Vect2d& round_this()
	{
#ifndef	__GCC_SUCKS__
		v=_mm_round_pd(v, _MM_FROUND_TO_NEAREST_INT|_MM_FROUND_NO_EXC);
#endif
		return *this;
	}
	Vect2d& round_this_sse2()
	{
		m128d_v0(v)=std::floor(m128d_v0(v)+0.5), m128d_vn(v, 1)=std::floor(m128d_vn(v, 1)+0.5);
		return *this;
	}
	Vect2d round()const
	{
	...
	{
		Vect2d x;
		m128d_v0(x.v)=std::floor(m128d_v0(v)+0.5), m128d_vn(x.v, 1)=std::floor(m128d_vn(v, 1)+0.5);
		return x;
	}
...
{
	__m128d x_q=_mm_div_pd(x.v, q.v);
	m128d_v0(x_q)=std::floor(m128d_v0(x_q)), m128d_vn(x_q, 1)=std::floor(m128d_vn(x_q, 1));
	x_q=_mm_mul_pd(x_q, q.v);
	x_q=_mm_sub_pd(x.v, x_q);
	return x_q;
}
...
/*	ALIGN(16) double lo2[2], hi2[2];//size multiple of 4, aligned by 16 bytes
...
/*	bool _2d_between(double x1, double y1, double x, double y, double x2, double y2)
	...
	namespace sse2
	{
		...
		__forceinline void assign(double *rr, Vect2d const &v)
		{
			STORE128D(rr, v);
		}
		__forceinline void assign(double *rr, double *ri, Comp2d const &v)
		{
			STORE128D(rr, v.r);
			STORE128D(ri, v.i);
		}
		__forceinline void assign(double *rr, double *ri, double *rj, double *rk, Quat2d const &v)
		{
			STORE128D(rr, v.r);
			STORE128D(ri, v.i);
			STORE128D(rj, v.j);
	...
	/*	void  r_r_erf					(VectP &r, VectP const &x)					{assign(r, Vect2d(boost::math::erf(x.lo()), boost::math::erf(x.hi()));}
		...
		{
			ALIGN(16) const Vect2d coef[6]={_mm_set1_pd(76.18009172947146), _mm_set1_pd(-86.50532032941677), _mm_set1_pd(24.01409824083091),//https://jamesmccaffrey.wordpress.com/2013/06/19/the-log-gamma-function-with-c/
	...
	//	void q_qq_hypot					(QuatP &r, QuatP const &x, QuatP const &y)	{Quat2d qx=x; assign(r, sqrt(sq(x)+sq(y));}
		...
		void conditional_111			(QuatP &res, QuatP const &op1, QuatP const &op2, QuatP const &op3, int idx, int op1_ms, int op_ms)
		{
			...
			switch(op_ms)
			{
			case 0:assign(res.r+idx,										Vect2d(op2.r+idx)&second.complement()									|	Vect2d(op3.r+idx)&second);break;//r_rr
			case 1:assign(res.r+idx, res.i+idx,							AND(Comp2d(op2.r+idx, op2.i+idx), second.complement())						|	Vect2d(op3.r+idx)&second);break;//c_rc
			case 2:assign(res.r+idx, res.i+idx, res.j+idx, res.k+idx,	AND(Quat2d(op2.r+idx, op2.i+idx, op2.j+idx, op2.k+idx), second.complement())|	Vect2d(op3.r+idx)&second);break;//q_rq
			case 3:assign(res.r+idx, res.i+idx,								Vect2d(op2.r+idx)&second.complement()									|AND(Comp2d(op3.r+idx, op3.i+idx), second));break;//c_cr
			case 4:assign(res.r+idx, res.i+idx,							AND(Comp2d(op2.r+idx, op2.i+idx), second.complement())						|AND(Comp2d(op3.r+idx, op3.i+idx), second));break;//c_cc
			case 5:assign(res.r+idx, res.i+idx, res.j+idx, res.k+idx,	AND(Quat2d(op2.r+idx, op2.i+idx, op2.j+idx, op2.k+idx), second.complement())|AND(Comp2d(op3.r+idx, op3.i+idx), second));break;//q_cq
			case 6:assign(res.r+idx, res.i+idx, res.j+idx, res.k+idx,		Vect2d(op2.r+idx)&second.complement()									|AND(Quat2d(op3.r+idx, op3.i+idx, op3.j+idx, op3.k+idx), second));break;//q_qr
			case 7:assign(res.r+idx, res.i+idx, res.j+idx, res.k+idx,	AND(Comp2d(op2.r+idx, op2.i+idx), second.complement())						|AND(Quat2d(op3.r+idx, op3.i+idx, op3.j+idx, op3.k+idx), second));break;//q_qc
			case 8:assign(res.r+idx, res.i+idx, res.j+idx, res.k+idx,	AND(Quat2d(op2.r+idx, op2.i+idx, op2.j+idx, op2.k+idx), second.complement())|AND(Quat2d(op3.r+idx, op3.i+idx, op3.j+idx, op3.k+idx), second));break;//q_qq
			}
		}
		...
		void va_min						(void *pv, void*, ArgIdx result, std::vector<ArgIdx> const &args, int idx)
		{
			...
			if(args.size())
			{
				...
				store2(n[result.idx], idx, result.mathSet, Quat2d(m_qnan, m_qnan, m_qnan, m_qnan));
		}
...
namespace	modes
{
	void colorFunction_bcw_sse2(CompP const &_v, int *rgb)
	{
		...
		Vect2d hyp=v.abs(), costh=v.r/hyp, sinth=v.i/hyp,
			mag=_FF*Vect2d(exp(-hyp*Vect2d(G2::_ln2*inv_th)));
		Vect2d
