...
#ifdef __GNUC__
inline double& m256d_v0(__m256d &v)
{
	return *(double*)&v;//
}
inline double m256d_v0(__m256d const &v)
{
	return *(double*)&v;//
}
inline double& m256d_vn(__m256d &v, int component)
{
	return ((double*)&v)[component];//
}
inline double m256d_vn(__m256d const &v, int component)
{
	return ((double*)&v)[component];//
}
#else
inline double& m256d_v0(__m256d &v){return v.m256d_f64[0];}
inline double m256d_v0(__m256d const &v){return v.m256d_f64[0];}
inline double& m256d_vn(__m256d &v, int component){return v.m256d_f64[component];}
inline double m256d_vn(__m256d const &v, int component){return v.m256d_f64[component];}
#endif
...
//	Vect4d():v(_mm256_setzero_pd()){}//CRASH on non-AVX CPU
	...
	double& v0(){return m256d_v0(v);}
	double& v1(){return m256d_vn(v, 1);}
	double& v2(){return m256d_vn(v, 2);}
	double& v3(){return m256d_vn(v, 3);}
	double v0()const{return m256d_v0(v);}
	double v1()const{return m256d_vn(v, 1);}
	double v2()const{return m256d_vn(v, 2);}
	double v3()const{return m256d_vn(v, 3);}
	double& get(int component){return m256d_vn(v, component);}
	double get(int component)const{return m256d_vn(v, component);}
...
void avx_initialize()
{
	...
	m_todeg=1./G2::_pi_180, m_torad=G2::_pi_180;
...
namespace	G2
{
	namespace avx
	{
		...
		__forceinline void assign(double *rr, Vect4d const &x){STORE256D(rr, x.v);}
		__forceinline void assign(double *rr, double *ri, Comp4d const &x)
		{
			STORE256D(rr, x.r.v);
			STORE256D(ri, x.i.v);
		}
		__forceinline void assign(double *rr, double *ri, double *rj, double *rk, Quat4d const &x)
		{
			STORE256D(rr, x.r.v);
			STORE256D(ri, x.i.v);
			STORE256D(ri, x.j.v);
	...
	/*	void  r_r_erf					(VectP &r, VectP const &x)					{assign(r, Vect4d(boost::math::erf(x.lo()), boost::math::erf(x.hi()));}
		...
		{
			ALIGN(32) const Vect4d coef[6]={_mm256_set1_pd(76.18009172947146), _mm256_set1_pd(-86.50532032941677), _mm256_set1_pd(24.01409824083091),//https://jamesmccaffrey.wordpress.com/2013/06/19/the-log-gamma-function-with-c/
	...
	/*	Vect4d  r_r_factorial				(Vect4d const &x)					{return Vect4d(tgamma(x.lo()+1), tgamma(x.hi()+1));}
		...
		__forceinline Vect4d todeg(Vect4d const &x){return x*m_todeg;}
		__forceinline Comp4d todeg(Comp4d const &x){return Comp4d(x.r*m_todeg, x.i);}
		__forceinline Quat4d todeg(Quat4d const &x){return Quat4d(x.r*m_todeg, x.i, x.j, x.k);}
		__forceinline Vect4d torad(Vect4d const &x){return x*m_torad;}
		__forceinline Comp4d torad(Comp4d const &x){return Comp4d(x.r*m_torad, x.i);}
		__forceinline Quat4d torad(Quat4d const &x){return Quat4d(x.r*m_torad, x.i, x.j, x.k);}
		void  r_r_cosd					(VectP &r, VectP const &x)					{assign(r, Vect4d(::cos(torad(x).v)));VLEAVE;}
		void  c_c_cosd					(CompP &r, CompP const &x)					{assign(r, cos(torad(Comp4d(x))));VLEAVE;}
		void  q_q_cosd					(QuatP &r, QuatP const &x)					{assign(r, cos(torad(Quat4d(x))));VLEAVE;}

		void  c_c_acosd					(CompP &r, CompP const &x)					{assign(r, todeg(acos(Comp4d(x))));VLEAVE;}
		void  q_q_acosd					(QuatP &r, QuatP const &x)					{assign(r, todeg(acos(Quat4d(x))));VLEAVE;}
		...
		void  r_r_secd					(VectP &r, VectP const &x)					{assign(r, m_one/Vect4d(::cos(torad(Vect4d(x)).v)));VLEAVE;}
		void  c_c_secd					(CompP &r, CompP const &x)					{assign(r, m_one/cos(torad(Comp4d(x))));VLEAVE;}
		void  q_q_secd					(QuatP &r, QuatP const &x)					{assign(r, m_one/cos(torad(Quat4d(x))));VLEAVE;}

		void  c_c_asecd					(CompP &r, CompP const &x)					{assign(r, todeg(acos(m_one/Comp4d(x))));VLEAVE;}
		void  q_q_asecd					(QuatP &r, QuatP const &x)					{assign(r, todeg(acos(m_one/Quat4d(x))));VLEAVE;}
		...
		void  r_r_sind					(VectP &r, VectP const &x)					{assign(r, Vect4d(::sin(torad(Vect4d(x)).v)));VLEAVE;}
		void  c_c_sind					(CompP &r, CompP const &x)					{assign(r, sin(torad(Comp4d(x))));VLEAVE;}
		void  q_q_sind					(QuatP &r, QuatP const &x)					{assign(r, sin(torad(Quat4d(x))));VLEAVE;}

		void  c_c_asind					(CompP &r, CompP const &x)					{assign(r, todeg(asin(Comp4d(x))));VLEAVE;}
		void  q_q_asind					(QuatP &r, QuatP const &x)					{assign(r, todeg(asin(Quat4d(x))));VLEAVE;}
		...
		void  r_r_cscd					(VectP &r, VectP const &x)					{assign(r, m_one/Vect4d(::sin(torad(Vect4d(x)).v)));VLEAVE;}
		void  c_c_cscd					(CompP &r, CompP const &x)					{assign(r, m_one/sin(torad(Comp4d(x))));VLEAVE;}
		void  q_q_cscd					(QuatP &r, QuatP const &x)					{assign(r, m_one/sin(torad(Quat4d(x))));VLEAVE;}

		void  c_c_acscd					(CompP &r, CompP const &x)					{assign(r, todeg(asin(m_one/Comp4d(x))));VLEAVE;}
		void  q_q_acscd					(QuatP &r, QuatP const &x)					{assign(r, todeg(asin(m_one/Quat4d(x))));VLEAVE;}
		...
		void  r_r_tand					(VectP &r, VectP const &x)					{assign(r, Vect4d(::tan(torad(Vect4d(x)).v)));VLEAVE;}
		void  c_c_tand					(CompP &r, CompP const &x)					{assign(r, tan(torad(Comp4d(x))));VLEAVE;}
		void  q_q_tand					(QuatP &r, QuatP const &x)					{assign(r, tan(torad(Quat4d(x))));VLEAVE;}

		void  r_r_atand					(VectP &r, VectP const &x)					{assign(r, todeg(Vect4d(::atan(Vect4d(x).v))));VLEAVE;}
		void  c_c_atand					(CompP &r, CompP const &x)					{assign(r, todeg(atan(Comp4d(x))));VLEAVE;}
		void  q_q_atand					(QuatP &r, QuatP const &x)					{assign(r, todeg(atan(Quat4d(x))));VLEAVE;}
		void r_rr_atand					(VectP &r, VectP const &y, VectP const &x)	{assign(r, todeg(Vect4d(::atan2(Vect4d(y).v, Vect4d(x).v))));VLEAVE;}
		...
		void  r_r_cotd					(VectP &r, VectP const &x)					{assign(r, m_one/Vect4d(::tan(torad(Vect4d(x)).v)));VLEAVE;}
		void  c_c_cotd					(CompP &r, CompP const &x)					{assign(r, m_one/tan(torad(Comp4d(x))));VLEAVE;}
		void  q_q_cotd					(QuatP &r, QuatP const &x)					{assign(r, m_one/tan(torad(Quat4d(x))));VLEAVE;}

		void  r_r_acotd					(VectP &r, VectP const &x)
		{
			Vect4d rx=x, mask=rx==m_zero;
			assign(r, todeg(Vect4d(::atan((m_one/rx).v))&mask.complement()|m_pi_2&mask));
			VLEAVE;
		}
	...
	//	void q_qq_hypot					(QuatP &r, QuatP const &x, QuatP const &y)	{Quat4d qx=x; assign(r, sqrt(sq(x)+sq(y));}
		...
		void conditional_111			(QuatP &res, QuatP const &op1, QuatP const &op2, QuatP const &op3, int idx, int op1_ms, int op_ms)
		{
			...
			switch(op_ms)
			{
			case 0:assign(res.r+idx,										Vect4d(op2.r+idx)&second.complement()									|	Vect4d(op3.r+idx)&second);break;//r_rr
			case 1:assign(res.r+idx, res.i+idx,							AND(Comp4d(op2.r+idx, op2.i+idx), second.complement())						|	Vect4d(op3.r+idx)&second);break;//c_rc
			case 2:assign(res.r+idx, res.i+idx, res.j+idx, res.k+idx,	AND(Quat4d(op2.r+idx, op2.i+idx, op2.j+idx, op2.k+idx), second.complement())|	Vect4d(op3.r+idx)&second);break;//q_rq
			case 3:assign(res.r+idx, res.i+idx,								Vect4d(op2.r+idx)&second.complement()									|AND(Comp4d(op3.r+idx, op3.i+idx), second));break;//c_cr
			case 4:assign(res.r+idx, res.i+idx,							AND(Comp4d(op2.r+idx, op2.i+idx), second.complement())						|AND(Comp4d(op3.r+idx, op3.i+idx), second));break;//c_cc
			case 5:assign(res.r+idx, res.i+idx, res.j+idx, res.k+idx,	AND(Quat4d(op2.r+idx, op2.i+idx, op2.j+idx, op2.k+idx), second.complement())|AND(Comp4d(op3.r+idx, op3.i+idx), second));break;//q_cq
			case 6:assign(res.r+idx, res.i+idx, res.j+idx, res.k+idx,		Vect4d(op2.r+idx)&second.complement()									|AND(Quat4d(op3.r+idx, op3.i+idx, op3.j+idx, op3.k+idx), second));break;//q_qr
			case 7:assign(res.r+idx, res.i+idx, res.j+idx, res.k+idx,	AND(Comp4d(op2.r+idx, op2.i+idx), second.complement())						|AND(Quat4d(op3.r+idx, op3.i+idx, op3.j+idx, op3.k+idx), second));break;//q_qc
			case 8:assign(res.r+idx, res.i+idx, res.j+idx, res.k+idx,	AND(Quat4d(op2.r+idx, op2.i+idx, op2.j+idx, op2.k+idx), second.complement())|AND(Quat4d(op3.r+idx, op3.i+idx, op3.j+idx, op3.k+idx), second));break;//q_qq
			}
			VLEAVE;
		}
		...
		inline Quat4d load4(Term &n, int idx, char ms)
	//	inline Quat4d load4(double *pr, double *pi, double *pj, double *pk, int idx, char ms)
		{
			Quat4d ret;
			ret.r=LOAD256D(n.r.p+idx);
			//if(n.i.p)
			//	ret.i=LOAD256D(n.i.p+idx);
			//else
			//	ret.i.setzero();
			//if(n.j.p)
			//	ret.j=LOAD256D(n.j.p+idx);
			//else
			//	ret.j.setzero();
			//if(n.k.p)
			//	ret.k=LOAD256D(n.k.p+idx);
			//else
			//	ret.k.setzero();
			if(ms>='c')
				ret.i=LOAD256D(n.i.p+idx);
		...
		inline void store4(Term &n, int idx, char ms, Quat4d const &v)
	//	inline void store4(double *pr, double *pi, double *pj, double *pk, int idx, char ms, Quat4d const &v)
		{
			STORE256D(n.r.p+idx, v.r);
			if(ms>='c')
				STORE256D(n.i.p+idx, v.i);
			if(ms=='h')
			{
				STORE256D(n.j.p+idx, v.j);
				STORE256D(n.k.p+idx, v.k);
			}
		}
		inline void clamp(Quat4d const &lo, Quat4d &x, Quat4d const &hi)
		{
			x.r=_mm256_max_pd(x.r, lo.r);
			x.i=_mm256_max_pd(x.i, lo.i);
			x.j=_mm256_max_pd(x.j, lo.j);
			x.k=_mm256_max_pd(x.k, lo.k);
			x.r=_mm256_min_pd(x.r, hi.r);
			x.i=_mm256_min_pd(x.i, hi.i);
			x.j=_mm256_min_pd(x.j, hi.j);
			x.k=_mm256_min_pd(x.k, hi.k);
		}
			...
			case 1:
				{
					...
					Quat4d
						lo,
						x =load4(n[args[0].idx], idx, args[0].mathSet),
						hi(one, one, one, one);
					lo.setzero();
					clamp(lo, x, hi);
					store4(n[result.idx], idx, result.mathSet, x);
				}
				break;
			case 2:
				{
					Quat4d
						lo,
						x =load4(n[args[0].idx], idx, args[0].mathSet),
						hi=load4(n[args[1].idx], idx, args[1].mathSet);
					lo.setzero();
					clamp(lo, x, hi);
					store4(n[result.idx], idx, (char)result.mathSet, x);
				}
				break;
			case 3:
				{
					Quat4d
						lo=load4(n[args[0].idx], idx, args[0].mathSet),
						x =load4(n[args[1].idx], idx, args[1].mathSet),
						hi=load4(n[args[2].idx], idx, args[2].mathSet);
					clamp(lo, x, hi);
					store4(n[result.idx], idx, result.mathSet, x);
				}
				break;
		...
		void va_min						(void *pv, void*, ArgIdx result, std::vector<ArgIdx> const &args, int idx)
		{
			...
			if(args.size())
			{
				int kn=args[0].idx;
				auto &n0=n[kn];
				Quat4d ret=load4(n0, idx, args[0].mathSet);
				for(int k=1;k<(int)args.size();++k)
				{
					kn=args[k].idx;
					auto &nk=n[kn];
					Quat4d vk=load4(nk, idx, args[k].mathSet);
					ret.r=_mm256_min_pd(ret.r, vk.r);
					ret.i=_mm256_min_pd(ret.i, vk.r);
					ret.j=_mm256_min_pd(ret.j, vk.r);
					ret.k=_mm256_min_pd(ret.k, vk.r);
				}
				store4(nr, idx, result.mathSet, ret);
			}
			else
				store4(nr, idx, result.mathSet, Quat4d(m_qnan, m_qnan, m_qnan, m_qnan));
			VLEAVE;
		}
		void va_max						(void *pv, void*, ArgIdx result, std::vector<ArgIdx> const &args, int idx)
		{
			...
			if(args.size())
			{
				...
				for(int k=1;k<(int)args.size();++k)
				{
					...
					auto &nk=n[kn];
					Quat4d vk=load4(nk, idx, args[k].mathSet);
					ret.r=_mm256_max_pd(ret.r, vk.r);
					ret.i=_mm256_max_pd(ret.i, vk.r);
					ret.j=_mm256_max_pd(ret.j, vk.r);
					ret.k=_mm256_max_pd(ret.k, vk.r);
				}
				store4(nr, idx, result.mathSet, ret);
			}
			else
		...
		void va_average					(void *pv, void*, ArgIdx result, std::vector<ArgIdx> const &args, int idx)
		{
			...
			for(int k=0;k<(int)args.size();++k)
				ret+=load4(n[args[k].idx], idx, args[k].mathSet);
			ret*=1./args.size();
			store4(n[result.idx], idx, result.mathSet, ret);
			VLEAVE;
		}
		void va_hypot					(void *pv, void*, ArgIdx result, std::vector<ArgIdx> const &args, int idx)
		{
			...
			for(int k=0;k<(int)args.size();++k)
			{
				...
				Quat4d term=load4(n[args[k].idx], idx, ms);
				switch(ms)
				{
				case 'R':
					ret.r+=term.r*term.r;
			...
			case 'R':
				...
				break;
			case 'c':
				ret=sqrt(Comp4d(ret.r, ret.i));
				break;
			case 'h':
				ret=sqrt(ret);
				break;
			}
			store4(n[result.idx], idx, rms, ret);
			VLEAVE;
		}
		void va_norm					(void *pv, void*, ArgIdx result, std::vector<ArgIdx> const &args, int idx)
		{
			...
			if(args.size())
			{
				...
				int kp=args[0].idx;
				Quat4d p=load4(n[kp], idx, pms), ret;
				ret.setzero();
				switch(pms)
				{
				case 'R':
					{
					//	Vect4d p=LOAD256D(n[kp].r.p+idx);
						for(int k=1;k<(int)args.size();++k)
						{
							char ms=args[k].mathSet;
							Quat4d term=load4(n[args[k].idx], idx, ms);
				...
				case 'c':
					{
						auto &np=n[kp];
						Comp4d cp(p.r, p.i);
					//	Comp4d p(LOAD256D(np.r.p+idx), LOAD256D(np.i.p+idx));
						for(int k=1;k<(int)args.size();++k)
						{
							char ms=args[k].mathSet;
							Quat4d term=load4(n[args[k].idx], idx, ms);
				...
				store4(n[result.idx], idx, result.mathSet, Quat4d(m_qnan, m_qnan, m_qnan, m_qnan));
			VLEAVE;
		}
	}//namespace sse2
}//namespace G2
