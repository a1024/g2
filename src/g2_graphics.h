...
//vector algebra:
extern const float inv255, infinity;
...
template<typename Type>inline bool			operator!=(tvec2<Type> const &a, tvec2<Type> const &b){return a.x!=b.x||a.y!=b.y;}
template<typename Type>inline tvec2<Type>	operator-(tvec2<Type> const &p){return tvec2<Type>(-p.x, -p.y);}
template<typename Type>struct	tmat2
{
	...
	void set_v(tvec2<Type> const &col1, tvec2<Type> const &col2)
	{
		a=col1.x, b=col2.x;
		c=col1.y, d=col2.y;
	}
...
struct			Bitmap
{
	int			w, h, *rgb;
	HBITMAP		hBitmap;
	Bitmap():w(0), h(0), rgb(0), hBitmap(nullptr){}
...
void			point_3D(Camera const &cam, dvec3 const &p_world, int lineColor);
void			point_3D_cam(Camera const &cam, dvec3 const &cp, int lineColor);//software line API
void			point_3D_2x2(Camera const &cam, dvec3 const &p_world, int Rcolor);//C3D
void			point_3D_2x2(Camera const &cam, dvec3 const &p_world, int Rcolor, int Icolor, int Jcolor, int Kcolor);//C3D
