...
namespace		GL2_2D
{
	...
	void		draw_line(float x1, float y1, float x2, float y2)
	{
		...
		gl_setProgram(program);					check(__LINE__);
		send_color(u_color, pen_color);			check(__LINE__);
		glEnableVertexAttribArray(a_vertices);	check(__LINE__);
	...
	//	g_fbuf[10]=X1, g_fbuf[11]=Y1;
		gl_setProgram(program);						check(__LINE__);
		send_color(u_color, 0xFF000000|pen_color);	check(__LINE__);
	...
	//	glDrawArrays(GL_POINTS, 5, 1);			check(__LINE__);
	}
...
namespace		GL2_L3D//light 3D
{
	...
	//	vbo_to_clipboard(&GL2_L3D::vertices[0], GL2_L3D::vertices.size(), &GL2_L3D::indices[0], GL2_L3D::indices.size(), 0);
		glBindBuffer(GL_ARRAY_BUFFER, GL2_L3D::VBO);															check(__LINE__);
		glBufferData(GL_ARRAY_BUFFER, (int)vertices.size()*sizeof(vec3), &GL2_L3D::vertices[0], GL_STATIC_DRAW);check(__LINE__);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, GL2_L3D::EBO);													check(__LINE__);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, (int)indices.size()<<2, &GL2_L3D::indices[0], GL_STATIC_DRAW);	check(__LINE__);
	}
	void		draw(Camera const &cam, vec3 const &lightpos)
	{
		...
		glUniform3fv(GL2_L3D::u_lightpos, 1, &lightpos.x);							check(__LINE__);
		vec3 cam_p=cam.p;
		glUniform3fv(GL2_L3D::u_viewpos, 1, &cam_p.x);								check(__LINE__);
		...
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, GL2_L3D::EBO);									check(__LINE__);
		glDepthMask(GL_FALSE);														check(__LINE__);//all L3D objects are transparent for now

		for(int k=0, kEnd=drawInfo.size(), start=0;k<kEnd;++k)
		{
			auto &dk=GL2_L3D::drawInfo[k];
			send_color(GL2_L3D::u_objectcolor, dk.color);										check(__LINE__);
			glDrawElements(GL_TRIANGLES, dk.count, GL_UNSIGNED_INT, (void*)(start<<2));			check(__LINE__);//use glDrawElementsBaseVertex
			start+=dk.count;
		}
		...
		//send_color(GL2_L3D::u_objectcolor, drawInfo[0].color);						check(__LINE__);//
		//glDrawElements(GL_TRIANGLES, sum, GL_UNSIGNED_INT, 0);						check(__LINE__);//

		glDepthMask(GL_TRUE);														check(__LINE__);
		glDisableVertexAttribArray(GL2_L3D::a_vertices);							check(__LINE__);
		glDisableVertexAttribArray(GL2_L3D::a_normals);								check(__LINE__);
	}
	...
	//	glPointSize(10);															check(__LINE__);
		...
		glBindBuffer(GL_ARRAY_BUFFER, buffer.VBO);												check(__LINE__);
		glVertexAttribPointer(GL2_L3D::a_vertices, 3, GL_FLOAT, GL_FALSE, buffer.vertices_stride, (void*)buffer.vertices_start);check(__LINE__);
		glEnableVertexAttribArray(GL2_L3D::a_vertices);											check(__LINE__);
		glVertexAttribPointer(GL2_L3D::a_normals, 3, GL_FLOAT, GL_FALSE, buffer.normals_stride, (void*)buffer.normals_start);	check(__LINE__);
		glEnableVertexAttribArray(GL2_L3D::a_normals);											check(__LINE__);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, buffer.EBO);										check(__LINE__);

		glDrawElements(GL_TRIANGLES, buffer.n_vertices, GL_UNSIGNED_INT, 0);		check(__LINE__);
...
namespace		GL2_3D
{
	...
	void		draw(Camera const &cam)
	{
		...
		for(int k=0, p_idx=0, kEnd=drawInfo.size();k<kEnd;++k)
		{
			...
			send_color(GL2_3D::u_color, di.textured?0xFFFF00FF:di.color);		check(__LINE__);//dummy color if textured
			if(di.textured)
			{
...
//	glUniform1i(GL2_Text::u_isTexture, false);		check(__LINE__);
	int msg_width=0;
	float _2_w=2.f/w, _2_h=2.f/h;
	...
	glVertexAttribPointer(GL2_Text::a_coord2d, 4, GL_FLOAT, GL_FALSE, 0, 0);check(__LINE__);
...
namespace		GL2_TI2D
{
	...
	#define	NPOT_ATIX2300_FIX
			...
			gl_setProgram(GL2_TI2D::program);					check(__LINE__);//select TI2D program
			send_color(GL2_TI2D::u_curvecolor, curvecolor);		check(__LINE__);//send color
			glUniform2f(GL2_TI2D::u_invs, 1.f/txw2, 1.f/txh2);	check(__LINE__);
			if(!tx_id)
				{glGenTextures(1, &tx_id);													check(__LINE__);}//generate texture id once
			glBindTexture(GL_TEXTURE_2D, tx_id);											check(__LINE__);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);				check(__LINE__);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);				check(__LINE__);
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, txw2, txh2, 0, GL_RED, GL_FLOAT, ndr2);	check(__LINE__);//send ndr to GPU

			select_texture(tx_id, GL2_TI2D::u_ndr);											check(__LINE__);
			glBindBuffer(GL_ARRAY_BUFFER, 0);												check(__LINE__);
			glVertexAttribPointer(GL2_TI2D::a_coords, 4, GL_FLOAT, GL_FALSE, 4<<2, vrtx);	check(__LINE__);//send vertices & texcoords

			glEnableVertexAttribArray(GL2_TI2D::a_coords);		check(__LINE__);
			glDrawArrays(GL_QUADS, 0, 4);						check(__LINE__);//draw the quad
			glDisableVertexAttribArray(GL2_TI2D::a_coords);		check(__LINE__);
#ifdef NPOT_ATIX2300_FIX
			if(expand)
				free(ndr2);
#endif
