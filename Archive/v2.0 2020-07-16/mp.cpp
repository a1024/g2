...
namespace		MP
{
	...
	void  c_c_sgn					(Quat &r, Quat const &x)				{Comp cx=(Comp)x; Real mag=abs(cx); r=mag?cx/mag:Comp();}
	...
	void  r_r_sinc					(Quat &r, Quat const &x)				{r=x.r?sin(x.r)/x.r:1;}
	...
	void  r_r_sinhc					(Quat &r, Quat const &x)				{r=x.r?sinh(x.r)/x.r:1;}
	...
	void  r_r_tanc					(Quat &r, Quat const &x)				{r=x.r?tan(x.r)/x.r:0;}
	...
	void  r_r_acot					(Quat &r, Quat const &x)				{r=x.r?atan(1/x.r):m_pi/2;}
...
//	inline Real	sgn			(Real const &x){return (x>0)-(x<0);}
	...
	void r_rr_conditional_110		(Quat &r, Quat const &x, Quat const &y)	{r=x.r?y.r:0;}
	void c_rc_conditional_110		(Quat &r, Quat const &x, Quat const &y)	{r=x.r?(Comp)y:Comp();}
	void q_rq_conditional_110		(Quat &r, Quat const &x, Quat const &y)	{r=x.r?y:Quat();}
	...
	void r_rr_conditional_101		(Quat &r, Quat const &x, Quat const &y)	{r=x.r?0:y.r;}
	void c_rc_conditional_101		(Quat &r, Quat const &x, Quat const &y)	{r=x.r?Comp():(Comp)y;}
	void q_rq_conditional_101		(Quat &r, Quat const &x, Quat const &y)	{r=x.r?Quat():y;}
